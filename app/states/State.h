...
namespace fsm
{
  class State
  {
    public:
      ...
      std::unique_ptr<State> poll(const int width, const int height, const ImGuiIO& io, const bool allEventsReady, evd::Window& window);
    ...
    protected:
      ...
      virtual std::unique_ptr<State> doPoll(const bool allEventsReady, evd::Window& window) = 0;
